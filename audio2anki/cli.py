...
@click.command()
@click.argument("audio_file", required=False, type=str)
...
def main(
    ...
    """
    Generate an Anki package (.apkg) from an audio file (or YouTube URL) and a
    Whisper-generated TSV transcript.

    AUDIO_FILE may be either a path to a local audio file or a YouTube URL
    (starting with http:// or https://). When a URL is given, audio is downloaded
    to ./youtube/ and the pipeline continues normally. The --youtube option still
    works as an explicit alternative.

    If a transcript TSV file is not provided, the script checks the transcripts
    directory for a file matching the audio file name and validates its duration
    against the audio. If no valid transcript is found (or if --whisper is passed),
    one is generated using Whisper.
    """
    ...
    if youtube:
        ...
        # then walk a fallback list before giving up.
        FALLBACK_PLAYER_CLIENTS = ["tv", "ios", "web_safari", "android_vr", "mweb"]

        def build_download_cmd(player_client=None):
            cmd = [
                yt_dlp_path,
                "-f",
                "bestaudio",
            ...
            cmd.extend(
                ["-o", os.path.join(youtube_dir, "%(title)s.%(ext)s"), youtube]
            )
            return cmd

        click.echo("Downloading MP3 from YouTube...")
        download_succeeded = False
        for client in [None] + FALLBACK_PLAYER_CLIENTS:
            label = "default player client" if client is None else f"player_client={client}"
            click.echo(f"Attempting download with {label}...")
            try:
                subprocess.run(build_download_cmd(client), check=True)
                download_succeeded = True
                break
            except subprocess.CalledProcessError:
                click.echo(
                    f"Download with {label} failed. Trying next option...", err=True
                )
    ...
    # In non-YouTube mode, require an audio_file that exists on disk.
    if not audio_file:
        click.echo(
            "Error: provide a YouTube URL or path to an audio file as the first argument.",
            err=True,
        )
        return
